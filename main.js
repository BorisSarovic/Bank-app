...
function newTable() {
  ...
  text = '';
  for (var i = 0; i < db.length; i++) {
    text += '<tr><td>'+db[i].id+'</td><td>'+db[i].name+'</td><td>'+db[i].deposit+'</td><td>'+db[i].cCard+'</td><td><input type="button" value="edit" class="'+i+' tableBtns btn btn-small btn-warning"/></td><td><input type="button" value="delete" class="'+i+' tableBtns btn btn-small btn-danger"/></td></tr>'
  }
  tBody.innerHTML = text;
